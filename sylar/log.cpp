...
namespace sylar{
    ...
    LogEvent::LogEvent(const char* file,int32_t line,uint32_t eplase,uint32_t threadId,uint32_t fiberId,uint64_t time):
        m_file(file),m_line(line),m_eplase(eplase),m_threadId(threadId),m_fiberId(fiberId),m_time(time)
    {
    ...
    void LogFormatter::init(){
        ...
        for(size_t i = 0; i < m_pattern.size(); ++i){//对每一个进行处理
            ...
            while(n < m_pattern.size()){//每次只处理一个
                if(!isalpha(m_pattern[n]) && m_pattern[n] != '{' && m_pattern[n] != '}'){//如果当前 字符 既不是 字母也不是{ },则代表格式错误
                    break;
                }
            ...
            }else if(fmt_status == 2){//若有 fmt 信息，则与没有格式化信息一样处理
                if(!nstr.empty()){
